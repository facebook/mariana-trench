...
static void check_taint_config_consistency(const std::string& pattern, const TaintConfig& frame) {
  if (frame.kind() == nullptr) {
    LiteralModelConsistencyError::raise(fmt::format(
        "Model for regex `{}` must have a source kind.", pattern));
  }
}

static Taint create_taint(const std::string& pattern, const std::vector<TaintConfig>& sources) {
  Taint all_sources;
