  ...
  if ("Class" == target) {
    ...
    try {
      ...
      std::throw_with_nested(JsonValidationError(
          ...
          /* field */ std::nullopt,
          fmt::format(
              "valid annotation target (`Class`, `Method`, or "
              "`Argument(...)`), got `{}`",
              target)));
    }
  }
...
AnnotationTarget::Kind AnnotationTarget::kind() const {
  return kind_;
}

const RootTemplate& AnnotationTarget::argument() const {
  return *argument_;
}
...
AnnotationFeatureTemplate::AnnotationFeatureTemplate(
    ...
    : target_(std::move(target)),
      dex_type_(dex_type),
      tag_(tag),
      annotation_parameter_name_(
          annotation_parameter_name ? annotation_parameter_name
...
std::optional<AnnotationFeatureTemplate> AnnotationFeatureTemplate::from_json(
  ...
  return AnnotationFeatureTemplate{
      std::move(port), dex_type, tag, annotation_parameter_name};
}
...
const Feature* MT_NULLABLE AnnotationFeatureTemplate::instantiate(
  ...
  auto value = std::find_if(
      annotation_elements.begin(),
      annotation_elements.end(),
  ...
  return context.feature_factory->get_via_annotation_feature(
      user_feature_data, tag_);
}
