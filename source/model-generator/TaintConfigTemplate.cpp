...
TaintConfigTemplate TaintConfigTemplate::from_json(
  ...
  if (value.isMember("via_annotation")) {
    for (const auto& feature_value :
         JsonValidation::null_or_array(value, "via_annotation")) {
      auto template_feature =
          AnnotationFeatureTemplate::from_json(feature_value);
      if (template_feature.has_value()) {
        annotation_features.emplace_back(template_feature.value());
      }
    }
  }
  return TaintConfigTemplate{
      std::move(taint_config), std::move(annotation_features)};
}

TaintConfig TaintConfigTemplate::instantiate(
    const Method* method,
...
TaintConfig TaintConfigTemplate::instantiate(
  ...
  for (const auto& annotation_feature : annotation_features_) {
    const Feature* MT_NULLABLE new_feature =
        annotation_feature.instantiate(method, context, parameter_positions);
    if (new_feature) {
...
TaintConfigTemplate::TaintConfigTemplate(
