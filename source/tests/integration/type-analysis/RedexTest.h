...
namespace { // Copied from RedexTestUtils.h
...
struct RedexIntegrationTest : public test::Test {
 protected:
  std::string dex_file;
 ...
 public:
  ...
  void run_passes(
    ...
    if (pg_config) {
      pass_manager = std::make_unique<PassManager>(
          passes, std::move(pg_config), *conf, redex_options);
    } else {
      pass_manager =
  ...
  DexClass* find_class(const C& clazzes, const std::string& name) {
    const auto it = std::find_if(
        clazzes.begin(), clazzes.end(), [&name](const DexClass* cls) {
          return cls->str() == name;
        });
