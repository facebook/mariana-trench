...
TEST_F(CallPositionFramesTest, LocalTaintLeq) {
  auto context = test::make_empty_context();
...
TEST_F(CallPositionFramesTest, Propagate) {
  ...
  auto frames = CallPositionFrames{
      test::make_taint_config(
          test_kind_one,
          test::FrameProperties{
              .origins = MethodSet{two}, .call_kind = CallKind::origin()}),
      test::make_taint_config(
          test_kind_one,
          test::FrameProperties{
              .callee_port = AccessPath(Root(Root::Kind::Argument, 0)),
...
TEST_F(CallPositionFramesTest, PropagateDropFrames) {
  ...
  auto frames = CallPositionFrames{
      test::make_taint_config(
          test_kind_one,
          test::FrameProperties{
              .callee = one, .distance = 1, .call_kind = CallKind::callsite()}),
  };
  EXPECT_EQ(
      frames.propagate(
          /* callee */ two,
...
TEST_F(CallPositionFramesTest, Show) {
  ...
  EXPECT_EQ(
      ...
      "call_kind=Declaration, origins={`LOne;.one:()V`})),]),])),]");
